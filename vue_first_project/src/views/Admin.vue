...
<!--      <el-button color="#58931b" type="warning">导出</el-button>-->
    ...
    <div class="box-card table_pag">
      <el-table v-loading="data.loading" ref="tableRef"  :data="data.tableData" stripe max-height="600px">
        ...
        <el-table-column label="头像" #default="{row}">
          <el-avatar style="margin-right: 5px" :size="50" :src="row.avatar || data.circleUrl" />
        </el-table-column>
  ...
  <el-dialog width="30%"  @close="addClose" v-model="data.addFormVisible" title="添加管理员信息">
  ...
  <el-dialog width="30%" @close="data.form={}" v-model="data.editFormVisible" title="编辑管理员信息">
...
const updateConfirmClick=()=>{
  request.put("/api/admin/"+data.form.id,data.form).then(res=>{
    ElMessage({
      message: '更新成功',
      type: 'success',
    })
...
const handleConfirmDeleteClick=()=>{
  request.delete("/api/admin/"+data.form.id).then(res=>{
    ElMessage({
      message: '删除成功',
      type: 'success',
    })
...
const fromConfirmClick=()=>{
  ...
  addFromRef.value.validate((valid)=>{
    if (valid){
      request.post("/api/admin/",data.form).then(res=>{
        ElMessage({
          message: '添加成功',
          type: 'success',
        })
...
const confirmBatchDeleteClick=()=>{
  request.delete("/api/admin",{data:data.ids}).then(res=>{
    console.log(res)
    data.ids=[]
    ElMessage({
      message:"删除成功",
